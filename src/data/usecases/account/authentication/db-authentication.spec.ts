...
const makeSut = (): SutTypes => {
  ...
  const updateAccessRepositoryStub = mockUpdateAccessTokenRepository()
...
describe('DbAuthtenticase UseCase', () => {
  ...
  test('Should throw if UpdateAccessTokenRepository throws', async () => {
    const { sut, updateAccessRepositoryStub } = makeSut()
    jest.spyOn(updateAccessRepositoryStub, 'updateAccessToken').mockImplementationOnce(throwError)
    const promise = sut.auth(mockAuthentication())
    await expect(promise).rejects.toThrow()
  })
})
